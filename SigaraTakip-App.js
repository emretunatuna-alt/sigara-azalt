...
import {
  ...
  Modal, StyleSheet, SafeAreaView, StatusBar, KeyboardAvoidingView, Platform
} from 'react-native';
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setData(d);
      setGoals(g);
      setGoalInput({ daily:String(g.daily), weekly:String(g.weekly) });
      setNotifEnabled(n);
      setLoaded(true);
    })();
  }, []);
  ...
  const checkAndScheduleNotification = useCallback(async () => {
    try {
      ...
      if (!lastEntry || !lastEntry.ts) return;
      const now = Date.now();
      const altiSaat = 6 * 60 * 60 * 1000;
      if (now - lastEntry.ts < altiSaat) return;
  ...
  }, [goals.daily, data, today]);

  useEffect(() => {
    if (!loaded || !notifEnabled) return;
    checkAndScheduleNotification();
  }, [loaded, notifEnabled, checkAndScheduleNotification]);
  ...
  const saveLogEdit = () => {
    const h = editTimeH.padStart(2,'0');
    const m = editTimeM.padStart(2,'0');
    const newTime = `${h}:${m}`;
    setData(prev => {
      const d = prev[today] || { sigara:0, km:0, kalp:null, entries:[], kalpLog:[], sigaraLog:[] };
  ...
  const prevKalpLog = prevData.kalpLog || [];
  const prevKalpAvg = prevKalpLog.length>0 ? Math.round(prevKalpLog.reduce((a,b)=>a+b,0)/prevKalpLog.length) : null;
  const kalpDiff = todayKalpAvg && prevKalpAvg ? todayKalpAvg-prevKalpAvg : null;
  ...
  const aralikBarData = statsDays.map(date => {
            ...
            <View style={s.notifCard}>
              <View style={s.notifRow}>
                <View>
                  ...
                  <Text style={s.notifSub}>{notifEnabled ? 'Hatırlatma açık' : 'Giriş yapmadığında hatırlatalım mı?'}</Text>
                </View>
                <TouchableOpacity
        ...
        {tab==='stats' && (
          <View>
            ...
            <View style={s.chartCard}>
              ...
              <BarChart data={aralikBarData} color="#b5851a"/>
            </View>
